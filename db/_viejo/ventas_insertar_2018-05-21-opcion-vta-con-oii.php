...
<!-- Font Awesome style desde mi servidor -->
  ...
  <body>
    ...
    <div class="col-sm-6">
		<h5>Opción 2: Venta con Order_Item_Id</h5>
		
		<?php $insertGoTo2 = "ventas_insertar_web.php";
   		$insertGoTo2 .= (strpos($insertGoTo, '?')) ? "&" : "?";
    	$insertGoTo2 .= $_SERVER['QUERY_STRING'];?>
		<form method="post" name="form2" id="form2" action="<?php echo $insertGoTo2; ?>">
		<div class="input-group form-group">
              <span class="input-group-addon"><i class="fa fa-user fa-fw"></i></span>
              <select name="order_item_id" class="selectpicker form-control" data-live-search="true" data-size="5" autofocus>
				<?php ?>
				<?php $i = 0; do { ?>
                    <option <?php if($i == 0): echo 'selected'; endif; ?> data-content="<span class='label label-normal'>#<?php echo $row_rsPedidos['order_item_id']?></span> <span class='label label-primary'><?php echo $row_rsPedidos['nombre']?> <?php echo $row_rsPedidos['apellido']?></span> <span class='label label-info'><?php echo $row_rsPedidos['producto']?></span>"
                    value="<?php echo $row_rsPedidos['order_item_id']?>"><?php echo $row_rsPedidos['order_item_id']?> <?php echo $row_rsPedidos['nombre']?> <?php echo $row_rsPedidos['apellido']?> <?php echo $row_rsPedidos['producto']?>
                     </option>
				  <?php 
				  $i++;} while ($row_rsPedidos = mysql_fetch_assoc($rsPedidos));
                  $rows = mysql_num_rows($rsPedidos);
				  if($rows > 0) {
					  
					  mysql_data_seek($rsPedidos, 0);
					  $row_rsPedidos = mysql_fetch_assoc($rsPedidos);
					  }
					?>
				</select>
