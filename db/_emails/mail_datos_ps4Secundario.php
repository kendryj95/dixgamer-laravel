...
$colname_rsCuenta = "-1";
if (isset($_GET['c_id'])) {
  $colname_rsCuenta = (get_magic_quotes_gpc()) ? $_GET['c_id'] : addslashes($_GET['c_id']);
}
mysql_select_db($database_Conexion, $Conexion);
$query_rsCuenta = sprintf("SELECT ID, mail_fake, pass
FROM cuentas
WHERE ID = %s", $colname_rsCuenta);
$rsCuenta = mysql_query($query_rsCuenta, $Conexion) or die(mysql_error());
...
$mail->msgHTML(str_replace(array('[nombrecliente]','[apellidocliente]','[nombrejuego]','[vta_id]','[clientes_id]','[stock_id]','[cuentamail]','[cuentapass]'), array(utf8_decode($row_rsClient['nombre']),utf8_decode($row_rsClient['apellido']),$row_rsClient['titulo'],$row_rsClient['ID_ventas'],$row_rsClient['clientes_id'],$row_rsClient['stock_id'],$row_rsCuenta['mail_fake'],$row_rsCuenta['pass']), file_get_contents('mail_datos_ps4Secundario_contenido.php')), dirname(__FILE__));
...
if (!$mail->send()) {
    ...
    echo "<h3>Datos de la cuenta</h3> <p>enviados a </p><h2>".$row_rsClient['email']."</h2>";
