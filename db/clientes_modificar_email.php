...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
	$modificaciones = sprintf("INSERT INTO clientes_notas(clientes_id, Notas, Day, usuario ) SELECT ID, CONCAT('email anterior: ', email), '$date', '$vendedor' FROM clientes WHERE ID=%s AND email IS NOT NULL",GetSQLValueString($_POST['ID'], "int"));
	mysql_select_db($database_Conexion, $Conexion);
  $ResultadoModif = mysql_query($modificaciones, $Conexion) or die(mysql_error());
	
  $updateSQL = sprintf("UPDATE clientes SET email=%s WHERE ID=%s", 
...
<script src="http://ajax.aspnetcdn.com/ajax/jquery.validate/1.11.0/jquery.validate.min.js"></script>
    <title><?php $titulo = 'Modificar E-mail'; echo $titulo; ?></title>
  ...
  <body style="padding: 0px;">
    ...
    <form method="post" name="form1" action="<?php echo $editFormAction; ?>">
			...
			<div id="user-result-div" class="input-group form-group">
              ...
              <input value="<?php echo $row_rsClientes['email']; ?>" class="form-control" type="text" name="email" id="email" autocomplete="off" spellcheck="false" placeholder="Email" autofocus>
              <span class="input-group-addon"><em class="text-muted"><?php echo $row_rsClientes['email']; ?></em></span>
            </div>
