...
$query_rsEstado = "select t1.*, t2.Q_Stock FROM
(select
    p.ID,
...
LEFT JOIN
(SELECT stk.titulo, stk.consola, (COALESCE(Q_Stock,0) + COALESCE(Q_stk_sin_reset,0)) as Q_Stock FROM 
...
WHERE reg_price > 0 and sale_price > 0 and Q_stock < 4";
...
<?php do {
	...
	echo "<p><a class='label label-info' target='_blank' href='https://dixgamer.com/wp-admin/post.php?post=" . $ID . "&action=edit'>" . $producto.  "</a> <em class='badge badge-default'>$" . $oferta . "<span style='font-size:0.8em;'>/" . $precio . "</span></em> <em class='badge badge-normal'>" .$stock . " en stock</em></p>" ;
