...
mysql_select_db($database_Conexion, $Conexion);
$query_rsStock2 = sprintf("SELECT COUNT(*) as Q, cuentas_id FROM stock WHERE cuentas_id=%s GROUP BY cuentas_id", $colname_rsStock);
...
WHERE cuentas_id = %s
UNION ALL
SELECT 'contra' as concepto, Day, NULL as ID_stock, NULL as titulo, NULL as consola, cuentas_id, NULL as clientes_id, NULL as slot, NULL as ventas_Notas, NULL as apellido, NULL as nombre, NULL as email, new_pass, usuario FROM cta_pass WHERE cuentas_id =%s
UNION ALL
SELECT 'reset' as concepto, Day, NULL as ID_stock, NULL as titulo, NULL as consola, cuentas_id, NULL as clientes_id, NULL as slot, NULL as ventas_Notas, NULL as apellido, NULL as nombre, NULL as email, NULL as new_pass, usuario FROM reseteo WHERE cuentas_id = %s
UNION ALL
SELECT 'resetear' as concepto, Day, NULL as ID_stock, NULL as titulo, NULL as consola, cuentas_id, NULL as clientes_id, NULL as slot, NULL as ventas_Notas, NULL as apellido, NULL as nombre, NULL as email, NULL as new_pass, usuario FROM resetear WHERE cuentas_id = %s
UNION ALL
SELECT 'notas' as concepto, Day, NULL as ID_stock, NULL as titulo, NULL as consola, cuentas_id, NULL as clientes_id, NULL as slot, NULL as ventas_Notas, NULL as apellido, NULL as nombre, NULL as email, Notas as new_pass, usuario FROM cuentas_notas WHERE cuentas_id = %s
) AS listado
...
mysql_select_db($database_Conexion, $Conexion);
$query_rsSaldo = sprintf("SELECT ex_stock_id as stk_id, 'carga' as concepto, cuentas_id, costo, costo_usd, code, code_prov, n_order, usuario FROM saldo WHERE cuentas_id = %s
UNION ALL
...
mysql_select_db($database_Conexion, $Conexion);
$query_UltimaCta = "SELECT cta.ID, stock.titulo, stock.consola, stock.costo_usd, stock.cuentas_id FROM
(SELECT cuentas.ID, stock.titulo FROM cuentas LEFT JOIN stock ON cuentas.ID = stock.cuentas_id WHERE titulo IS NOT NULL and cuentas.usuario='$vendedor' ORDER BY ID DESC LIMIT 1) as cta
LEFT JOIN
stock
ON cta.ID = stock.cuentas_id
WHERE stock.titulo IS NOT NULL
ORDER BY ID DESC";
$UltimaCta = mysql_query($query_UltimaCta, $Conexion) or die(mysql_error());
$row_UltimaCta = mysql_fetch_assoc($UltimaCta);
$totalRows_UltimaCta = mysql_num_rows($UltimaCta);
?>
<!DOCTYPE html>
  ...
  <?php include('_barra_nav.php'); ?>
    ...
    <div class="row">
        ...
        <div class="panel panel-warning">
          ...
          <div class="panel-body" style="background-color: #efefef;">
			...
			<a class="btn btn-normal btn-xs pull-right" href="imacros://run/?m=abrir-sony.iim" style="opacity:0.5;">Abrir Sony</a>
    ...
    if($row_rsStock): ?>
        ...
        <div class="col-xs-12 col-sm-4 col-md-3">
            ...
            <p>
                <span class="btn-group pull-right">
					<button class="btn btn-xs btn-default" type="button" data-toggle="modal" data-target=".bs-example-modal-lg" onClick='document.getElementById("ifr").src="stock_modificar.php?s_id=<?php echo $row_rsStock['ID_stock']; ?>&c_id=<?php echo $row_rsStock['stock_cuentas_id']; ?>";'><i class="fa fa-pencil"></i></button>
                </span>
            ...
            <p style="opacity:0.7">
				<img src="https://exur-exur.netdna-ssl.com/realskin/images/usa.png" style="opacity:0.7"> <small><strong><?php echo $row_rsStock['costo_usd'];?></strong></small>
				<?php if (!($row_rsStock2['Q'] > 1)):?>
					<a title="Modificar Costo" class="btn-xs text-muted" style="opacity: 0.7;" type="button" data-toggle="modal" data-target=".bs-example-modal-lg" onClick='document.getElementById("ifr").src="stock_modificar_costo.php?s_id=<?php echo $row_rsStock['ID_stock']; ?>&c_id=<?php echo $row_rsStock['stock_cuentas_id']; ?>";'><i aria-hidden="true" class="fa fa-pencil"></i></a>
				<?php endif;?>
    ...
    <div class="col-md-2 pull-right">
		<p>
		 <button class="btn btn-default btn-lg" type="button" data-toggle="modal" data-target=".bs-example-modal-lg" onClick='document.getElementById("ifr").src="cuentas_saldo_pre_insertar.php?cta_id=<?php echo $row_rsCuentas['ID'];?>";'><i class="fa fa-fw fa-dollar"></i> Cargar Saldo</button>
	</p>
     <?php if ($saldo > 0.00):?>
	<p><button class="btn btn-primary btn-lg" type="button" data-toggle="modal" data-target=".bs-example-modal-lg" onClick='document.getElementById("ifr").src="cuentas_stock_insertar.php?cta_id=<?php echo $row_rsCuentas['ID'];?>";'><i class="fa fa-fw fa-gamepad"></i> Cargar Juego</button></p>
		
	<p><button class="btn btn-warning btn-lg" type="button" data-toggle="modal" data-target=".bs-example-modal-lg" onClick='document.getElementById("ifr").src="cuentas_stock_pre_insertar.php?cta_id=<?php echo $row_rsCuentas['ID'];?>&saldo=<?php echo $saldo;?>";'><i class="fa fa-fw fa-th"></i> Carga Masiva</button></p>
		...
		<div class="text-center" style="background-color:#f1f1f1; border-color:#fcfcfc; border: 1px solid; padding: 5px; margin: 5px 10px 5px 0">
			<?php do { ?>
              <div class="col-sm-5" >
				<div>
                 <img src="/img/productos/<?php echo $row_UltimaCta['consola']."/".$row_UltimaCta['titulo'].".jpg"; ?>" alt="<?php echo $row_UltimaCta['consola']." - ".$row_UltimaCta['titulo'].".jpg"; ?>" class="img img-responsive full-width" />
                 </div>
                <div class="caption text-center" style="margin:5px 0;">
                <span class="badge badge-normal"><?php echo $row_UltimaCta['costo_usd']; ?> usd</span>
				<?php $saldo_ultima_cta = $saldo_ultima_cta + $row_UltimaCta['costo_usd'];?>
                </div>
              </div>
			<?php $almaceno_ultima_cta_id = $row_UltimaCta['ID'];?>
			<?php } while ($row_UltimaCta = mysql_fetch_assoc($UltimaCta)); ?>
		<?php if ($saldo <  $saldo_ultima_cta):?><p class="badge badge-danger">Se necesita <?php echo $saldo_ultima_cta;?> usd</p>
		<?php else:?>
			<button class="btn btn-normal btn-xl" type="button" data-toggle="modal" data-target=".bs-example-modal-lg" onClick='document.getElementById("ifr").src="cuentas_stock_repetir.php?cta_id=<?php echo $row_rsCuentas['ID'];?>&ultima_cta_id=<?php echo $almaceno_ultima_cta_id;?>&saldo_usd=<?php echo $saldo;?>&saldo_ars=<?php echo $saldoARS;?>";'><i class="fa fa-fw fa-step-backward"></i> Igual Ultima Cta</button>
		<?php endif;?>
		</div>
		<?php endif;?>
    ...
    <thead>
          ...
          <?php do { ?><tr>
            ...
            <td colspan="4"><em class="badge badge-danger" style="font-weight:normal; opacity:0.8;"><i class="fa fa-power-off fa-fw" aria-hidden="true"></i> Reseteado por <?php echo $row_rsClient['usuario'];?></em></td>
