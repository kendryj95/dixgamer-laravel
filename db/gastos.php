...
if (isset($_GET['concepto'])) {
  $colname_rsGastito = (get_magic_quotes_gpc()) ? $_GET['concepto'] : addslashes($_GET['concepto']);
  mysql_select_db($database_Conexion, $Conexion);
	$query_rsClientes = sprintf("SELECT * FROM gastos WHERE concepto LIKE '%s' ORDER BY ID DESC", $colname_rsGastito);
...
if (!(isset($_GET['concepto']))) {
mysql_select_db($database_Conexion, $Conexion);
$query_rsClientes = "SELECT * FROM gastos ORDER BY ID DESC";
$query_limit_rsClientes = sprintf("%s LIMIT %d, %d", $query_rsClientes, $startRow_rsClientes, $maxRows_rsClientes);
$rsClientes = mysql_query($query_limit_rsClientes, $Conexion) or die(mysql_error());
$row_rsClientes = mysql_fetch_assoc($rsClientes);
}
...
mysql_select_db($database_Conexion, $Conexion);
$query_rsconcepto = "SELECT * FROM gastos GROUP BY concepto ORDER BY concepto ASC";
$rsconcepto = mysql_query($query_rsconcepto, $Conexion) or die(mysql_error());
...
<!-- InstanceBeginEditable name="doctitle" -->
    <title><?php $titulo = 'Listar gastos'; echo $titulo; ?></title>
<!-- InstanceEndEditable -->
...
<!-- InstanceEndEditable -->
  ...
  <?php include('_barra_nav.php'); ?>
    ...
    <!-- InstanceBeginEditable name="body" -->
        <a class="btn btn-success btn-sm" href="gastos.php" title="Ver Todos" style="margin:5px 0 0 0;">Todos</a>
		<?php do { ?>
        <a class="btn btn-default btn-sm" href="gastos.php?concepto=<?php echo $row_rsconcepto['concepto']; ?>" title="Filtrar <?php echo $row_rsconcepto['concepto']; ?>" style="margin:5px 0 0 0;">
        <?php echo $row_rsconcepto['concepto']; ?>
        </a>
        <?php } while ($row_rsconcepto = mysql_fetch_assoc($rsconcepto)); ?>
    ...
    <thead>
          ...
          <?php do { ?><tr>
          	...
          	<td><?php echo $row_rsClientes['ID']; ?></td>
            <td><?php echo date("d-M", strtotime($row_rsClientes['Day'])); ?></td>
            <td><?php echo $row_rsClientes['concepto']; ?></td>
            <td>$ <?php echo round($row_rsClientes['importe']); ?></td>
            <td><?php echo $row_rsClientes['nro_transac']; ?></td>
            <?php 
			if ($row_rsClientes['medio_pago'] == 'MercadoPago'): $color = 'primary';
			elseif ($row_rsClientes['medio_pago'] == 'Transferencia Bancaria'): $color = 'default';
			elseif ($row_rsClientes['medio_pago'] == 'Efectivo'): $color = 'success';
			elseif ($row_rsClientes['medio_pago'] == 'Tarjeta'): $color = 'info';
			endif;?>
            
            <td><span class="label label-<?php echo $color;?>"><?php echo $row_rsClientes['medio_pago']; ?></span></td>   
                <td><?php echo $row_rsClientes['Notas']; ?></td>     
          </tr>
