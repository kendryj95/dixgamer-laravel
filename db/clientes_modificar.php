...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
	$modificaciones = sprintf("INSERT INTO clientes_notas(clientes_id, Notas, Day, usuario ) SELECT ID, CONCAT('Cambio nombre ', apellido,' ', nombre, ' a ', %s, ' ', %s), '$date', '$vendedor' FROM clientes WHERE ID=%s AND nombre IS NOT NULL AND apellido IS NOT NULL",GetSQLValueString($_POST['apellido'], "text"),GetSQLValueString($_POST['nombre'], "text"),GetSQLValueString($_POST['ID'], "int"));
	mysql_select_db($database_Conexion, $Conexion);
  $ResultadoModif = mysql_query($modificaciones, $Conexion) or die(mysql_error());
	
  $updateSQL = sprintf("UPDATE clientes SET apellido=%s, nombre=%s WHERE ID=%s", 
...
<html lang="es">
  <head>
    ...
    <title><?php $titulo = 'Modificar Nombre'; echo $titulo; ?></title>
  ...
  <body>
    ...
    <form method="post" name="form1" action="<?php echo $editFormAction; ?>">
        ...
        <div class="input-group form-group">
              ...
              <input value="<?php echo $row_rsClientes['apellido']; ?>" class="form-control" type="text" name="apellido" placeholder="Apellido">
              <span class="input-group-addon"><em class="text-muted"><?php echo $row_rsClientes['apellido']; ?></em></span>
            </div>
            
            <div class="input-group form-group">
              <span class="input-group-addon"><i class="fa fa-user fa-fw"></i></span>
              <input value="<?php echo $row_rsClientes['nombre']; ?>" class="form-control" type="text" name="nombre" placeholder="Nombre">
              <span class="input-group-addon"><em class="text-muted"><?php echo $row_rsClientes['nombre']; ?></em></span>
            </div>
