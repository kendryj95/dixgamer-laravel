...
WHERE convideo.cv_meta_value IS NULL";
...
if ($row_rsEstado['post_id']) {
	...
	//inserto en la base de datos el campo personalizado _video_url con el valor del ID de YT
	if("" !== trim($video_id)){
	$updateSQL = "INSERT INTO cbgw_postmeta (post_id, meta_key, meta_value) VALUES ('$post_id', '$meta_key', '$video_id')";
	mysql_select_db($database_Conexion, $Conexion);
	$Result2 = mysql_query($updateSQL, $Conexion) or die(mysql_error());
	echo 'producto ' . $post_id. ' cargado video id <em>' . $video_id . '</em> a WS<br>' ;
	}
