...
class Stock extends Model
{
    ...
    public function soldFronConcept($id){
        return DB::select(DB::raw("
              SELECT *
              FROM
              (SELECT client.ID_ventas AS id, 'venta' as concepto, client.Day, client.Day_orig, ID AS ID_stock, titulo, consola, cuentas_id, clientes_id, slot, ventas_Notas, apellido, nombre, email,  NULL as new_pass, NULL as usuario, client.auto, client.recup
              FROM stock
              RIGHT JOIN
              (SELECT ventas.ID AS ID_ventas, clientes_id, stock_id, slot, medio_cobro, precio, comision, estado, ventas.Notas AS ventas_Notas, ventas.Day_modif AS Day, ventas.Day AS Day_orig, clientes.ID AS ID_clientes, apellido, nombre, email, clientes.auto, ventas.recup
              FROM ventas
              ...
              WHERE cuentas_id = $id
              UNION ALL
              SELECT ID AS id, 'contra' as concepto, Day, NULL AS Day_orig, NULL as ID_stock, NULL as titulo, NULL as consola, cuentas_id, NULL as clientes_id, NULL as slot, NULL as ventas_Notas, NULL as apellido, NULL as nombre, NULL as email, new_pass, usuario, NULL AS auto, NULL AS recup FROM cta_pass WHERE cuentas_id =$id
              UNION ALL
              SELECT ID AS id, 'reset' as concepto, Day, NULL AS Day_orig, NULL as ID_stock, NULL as titulo, NULL as consola, cuentas_id, NULL as clientes_id, NULL as slot, NULL as ventas_Notas, NULL as apellido, NULL as nombre, NULL as email, NULL as new_pass, usuario, NULL AS auto, NULL AS recup FROM reseteo WHERE cuentas_id = $id
              UNION ALL
              SELECT ID AS id, 'resetear' as concepto, Day, NULL AS Day_orig, NULL as ID_stock, NULL as titulo, NULL as consola, cuentas_id, NULL as clientes_id, NULL as slot, NULL as ventas_Notas, NULL as apellido, NULL as nombre, NULL as email, NULL as new_pass, usuario, NULL AS auto, NULL AS recup FROM resetear WHERE cuentas_id = $id
              UNION ALL
              SELECT ID AS id, 'notas' as concepto, Day, NULL AS Day_orig, NULL as ID_stock, NULL as titulo, NULL as consola, cuentas_id, NULL as clientes_id, NULL as slot, NULL as ventas_Notas, NULL as apellido, NULL as nombre, NULL as email, Notas as new_pass, usuario, NULL AS auto, NULL AS recup FROM cuentas_notas WHERE cuentas_id = $id
              UNION ALL
              (SELECT NULL AS id, 'referencia' as concepto, (SELECT fecha_referencia FROM configuraciones WHERE ID = 1) AS Day, NULL AS Day_orig, NULL as ID_stock, NULL as titulo, NULL as consola, NULL AS cuentas_id, NULL as clientes_id, NULL as slot, NULL as ventas_Notas, NULL as apellido, NULL as nombre, NULL as email, NULL as new_pass, NULL AS usuario, NULL AS auto, NULL AS recup)
              ) AS listado
              ORDER BY Day DESC
        "));
    }
    ...
    public function ScopeStockNotesList($query,$obj){
        ...
        ->select('sn.stock_id','sn.Notas','sn.Day','sn.usuario', 's.*', DB::raw("(SELECT color FROM usuarios WHERE Nombre = sn.usuario) AS color_user"))
        ->leftjoin(DB::raw("(SELECT stock.ID AS stock_id, stock.titulo, stock.consola, stock.cuentas_id, stock.code FROM stock
        UNION ALL 
        SELECT saldo.ex_stock_id AS stock_id, saldo.titulo, saldo.consola, saldo.cuentas_id, saldo.code FROM saldo) AS s"),'sn.stock_id','=','s.stock_id');
    ...
    public function scopeStockDisponibleWithIdStock($query, $consola, $titulo, $slot, $stk_id)
    {
        ...
        if (($consola && ($consola == "ps4")) or ($titulo && ($titulo == "plus-12-meses-slot"))){
            if ($slot && (ucwords($slot) == "Primario")) {
                $row_rsSTK = DB::select(DB::raw("SELECT ID AS ID_stk, titulo, consola, cuentas_id AS stk_ctas_id, round(AVG(costo_usd),2) as costo, ID_vta, Q_vta, dayvta, Q_vta_pri, Q_vta_sec, COUNT(*) AS Q_Stock
                        ...
                        GROUP BY stock_id) AS vendido
                        ON ID = stock_id
                        WHERE (consola = 'ps4' OR titulo = 'plus-12-meses-slot') AND (Q_vta_pri IS NULL) AND stock.ID=?
                        GROUP BY  consola, titulo
                        ORDER BY consola, titulo, ID DESC"), [$stk_id]);
                ...
                $row_rsSTK = DB::select(DB::raw("SELECT ID AS ID_stk, titulo, consola, cuentas_id AS stk_ctas_id, round(AVG(costo_usd),2) as costo, ID_vta, Q_vta, dayvta, Q_vta_pri, Q_vta_sec, COUNT(*) AS Q_Stock
                    ...
                    GROUP BY stock_id) AS vendido
                    ON ID = stock_id
                    WHERE (consola = 'ps4' OR titulo = 'plus-12-meses-slot') AND (Q_vta_sec IS NULL) AND stock.ID=?
                    GROUP BY  consola, titulo
                    ORDER BY consola, titulo, ID DESC"), [$stk_id]);
            ...
            $row_rsSTK = DB::select(DB::raw("SELECT ID_stk, titulo, consola, stk_ctas_id, dayreset, Q_reset, days_from_reset, Q_vta, round(AVG(costo_usd),2) as costo, SUM(Q_Stock) AS Q_Stock FROM (SELECT ID AS ID_stk, titulo, consola, round(AVG(costo_usd),2) as costo_usd, cuentas_id AS stk_ctas_id, ID_reseteo AS ID_reset, r_cuentas_id AS reset_ctas_id, dayreseteo AS dayreset, reset.Q_reseteado AS Q_reset, DATEDIFF(NOW(), dayreseteo) AS days_from_reset, ID_vta, Q_vta, dayvta, ((2 + (IFNULL(Q_reseteado, 0) * 2)) - IFNULL(Q_vta, 0)) AS Q_Stock
                ...
                WHERE stock.ID=?
                GROUP BY consola, titulo
                ORDER BY consola, titulo, ID_stk"), [$stk_id]);
            ...
            $row_rsSTK = DB::select(DB::raw("SELECT ID AS ID_stk, titulo, consola, cuentas_id AS stk_ctas_id, round(AVG(costo_usd),2) as costo, ID_vtas, Q_vta, dayvta, COUNT(*) AS Q_Stock
                ...
                ORDER BY ID DESC) AS vendido
                ON ID = stock_id
                WHERE (consola != 'ps4') AND (consola != 'ps3') AND (Q_vta IS NULL) AND (titulo != 'plus-12-meses-slot') AND stock.ID=?
                GROUP BY consola, titulo
                ORDER BY consola, titulo DESC"), [$stk_id]);
