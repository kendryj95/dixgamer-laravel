        ...
        $schedule->call(function () {
            ...
            try {
                ...
                DB::update("UPDATE stock s LEFT JOIN 
                            (SELECT ID, IF(sugerido<(costo_usd * 0.25),FORMAT((costo_usd * 0.25),2),sugerido) as costo_usd_a_modificar FROM (SELECT ID, costo_usd, FORMAT(costo_usd * (0.12 + POWER(0.985,FLOOR((datediff(CURDATE(), Day))/7))),2) as sugerido FROM stock where consola='ps4' and costo_usd>5 and (titulo LIKE '%fifa%' or titulo LIKE '%pes%') ORDER BY ID ASC) AS primer_calculo) as rdo
                            ON s.ID = rdo.ID
