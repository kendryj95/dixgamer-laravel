...
1
				-->
					@component('components.clipboards.account')
						@slot('clientes_id', $sc->clientes_id)
						@slot('pass', $account->pass)
						@slot('nombre_cliente', $sc->nombre)
						@slot('titulo', $sc->titulo)
						@slot('mail_fake', $account->mail_fake)
						@slot('account_name', $account->name)
						@slot('account_surname', $account->surname)
				...
				<?php elseif ($sc->consola == "ps" && $sc->titulo == "plus-12-meses-slot"): ?>
					...
					@if ($sc->slot == "Primario")
						@if ($sc->auto === "bloq")
							<span class="badge badge-danger"><i class="fa fa-times-circle"></i> Cliente bloqueado</span>
						@else
							<a href="#<?php echo $sc->clientes_id; ?>" class="btn-copiador btn-xs btn-info label" id="msj_react_pri" data-clipboard-target="#msjreact-copy{{$sc->clientes_id}}">msj react <i aria-hidden="true" class="fa fa-clone"></i></a>
			...
			<td class="text-center">
				@if($sc->concepto === "venta")
					@if ($sc->ID_stock != 1 && $sc->consola == 'ps4')
						<div class="dropdown" style="display: inline-block;">
							<button class="btn btn-default btn-xs dropdown-toggle" type="button" id="removeProduct_{{$sc->slot == 'Secundario' ? 'secu' : 'pri'}}" data-toggle="dropdown" aria-haspopup="true" aria-expanded="true" style="opacity: 0.5;background: transparent;border: none;padding-top: 0px;padding-bottom: 0px;padding-right: 0px;padding-left: 5px;">
