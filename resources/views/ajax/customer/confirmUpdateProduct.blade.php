<div class="container">
	...
	@else
			...
			<div class="row text-center" style="background-color:#cfcfcf;padding:5px; border: 1px dashed #efefef">
				<img class="img-rounded" width="60" src="{{asset('img/productos')}}/{{$consola}}/{{$titulo}}.jpg" alt="{{ $titulo }} - {{ $consola }}" /><h4 style="display: inline"> Asignar {{ $titulo }} ({{ $consola }})</h4>

				@if (($consola === "ps4") || ($titulo === "plus-12-meses-slot"))

					<em style="font-size:0.8m"> {{ $slot }} </em>
